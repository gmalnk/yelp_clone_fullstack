...
app.get("/api/v2/restaurants", async (req, res) => {
  ...
  try {
    const results = await db.query(
      "SELECT * FROM restaurants LEFT JOIN (SELECT restaurant_id, COUNT(rating) as count, TRUNC(AVG(rating),1) as average_rating from reviews GROUP BY restaurant_id ) reviews on restaurants.id = reviews.restaurant_id "
    );
    console.log(results);
    res.status(200).json({
      status: "success",
...
app.get("/api/v2/restaurants/:restaurants_id", async (req, res) => {
  console.log(req.params);
  try {
    const restaurants = await db.query(
      "SELECT * FROM restaurants LEFT JOIN (SELECT restaurant_id, COUNT(rating) as count, TRUNC(AVG(rating),1) as average_rating from reviews GROUP BY restaurant_id ) reviews on restaurants.id = reviews.restaurant_id WHERE id = $1",
      [req.params.restaurants_id]
    );
...
// create a restaurant
app.post("/api/v2/restaurants", async (req, res) => {
...
app.put("/api/v2/restaurants/:id", async (req, res) => {
  console.log(req.params);
  console.log(req.body);
  try {
    const results = await db.query(
...
app.delete("/api/v2/restaurants/:id", async (req, res) => {
  console.log(req.params);
  try {
...
app.post("/api/v2/restaurants/review/:id", async (req, res) => {
  try {
    const results = await db.query(
