            ...
            <table className="table table-hover table-dark">
                ...
                <tbody className="">
                    {restaurants && restaurants.map(restaurants =>{
                        return(<tr key = {restaurants.id} onClick={()=>{handleRestaurantSelect(restaurants.id)}}>
                            <td>{restaurants.name}</td>
                            <td>{restaurants.location}</td>
                            <td>{"$".repeat(restaurants.price_range)}</td>
                            <td>{handleStarComponent(restaurants)}</td>
                            <td><button className="btn btn-warning" onClick={(event)=> handleUpdate(event,restaurants.id)}>Update</button></td>
                            <td><button className="btn btn-danger" onClick={(event)=>handleDelete(event,restaurants.id)}>Delete</button></td>
                        </tr>)
                    })}
                    {/* <tr>
                        <td>mcd</td>
